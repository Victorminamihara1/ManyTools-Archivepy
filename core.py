...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    variants = {
        'data': ['data','date'],
        'loja_id': ['loja_id','loja','store_id','filial'],
        'produto_id': ['produto_id','produto','product_id','sku'],
        'quantidade': ['quantidade','qtd','quantity','qty'],
        'preco_unitario': ['preco_unitario','preco','preco_un','unit_price','valor_unitario'],
    }
...
def ler_e_preparar(diretorio_raiz: str):
    avisos = []
    p_root = Path(diretorio_raiz)
    p_plan = p_root / "planilha"
...
def gravar_sqlite(db_path: str, df: pd.DataFrame):
    if df.empty: return 0
    with sqlite3.connect(db_path) as con:
        df.to_sql("vendas", con, if_exists="append", index=False)
        return len(df)

def gerar_relatorio(diretorio_raiz: str, db_path: str, df: pd.DataFrame, avisos: list[str]) -> str:
    root = Path(diretorio_raiz)
